...
class BarCode128
{
    ...
    public function createBarCode($image_type = 'png', $show_txt = false, $file_name = null)
    {
        $this->parseCode();
        if (!$show_txt) {
            $this->font_height = 1;
        ...
        if ($show_txt) {
            ...
            for ($i = 0; $i < $t_num; $i++) {
                $n = $i + 1;
                $c = $i - floor($n / 5);
                $text = ($n % 5 == 0) ? '-' : $this->text[(int)$c];
                imagechar($this->image, $this->font_type, 6 * $this->unit_width + $t_sx + $i * $t_x, 0, $text, $this->color);
            }
        }
        if (!$file_name) {
